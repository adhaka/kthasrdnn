...
class DAE(object):
	def __init__(self, numpy_rng, theano_rng, inp, n_inputs, n_hiddens, w, bhid=None, bvis=None, corruption=0.30, config=1):
		...
		self.delta_b = theano.shared(value=np.zeros_like(self.b.get_value()), name='delta_b', borrow=True)
	...
	def get_reconstructed_output(self, hidden):
		return T.nnet.sigmoid(T.dot(self.hidden, self.w) + self.bvis)


	def get_cost_updates(self, corruption_level, lr =0.02, momentum=0.4):
		""" This function computes the cost update after one epoch of training."""
		corrupted_x = self.get_corrupted_input(x, corruption_level)
	...
	def get_corrupted_input(self, X, corruption_level):
		return self.theano_rng.binomial(size=X.shape, n=1, 1 - corruption_level, dtype=theano.config.floatX) * X 
