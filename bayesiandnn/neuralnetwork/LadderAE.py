...
class LadderAE(object):
	def __init__(self, x_labelled, x_unlabelled, y_labelled, num_output, hidden_layer_config, corruption_layer_config=[0.2], activation_layer_config=['tanh'], learning_rate_config=[0.02], BATCH_SIZE=400, random_seed=1111):
	...
	def encoder_pass(self, inputs, weight):
		...
		# clean encoder pass ...
		for i in xrange(self.num_layers):
			le = EncoderLadder(input_labelled, input_unlabelled, self.hidden_layer_config[i])
			le_corrupt = EncoderLadder(input_labelled_corrupt, input_unlabelled_corrupt, self.hidden_layer_config[i], noise_std=noise_std)
			encoders.append(le)
